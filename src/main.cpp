...
int main(int argc, char *argv[])
{
    ...
    // This thing is needed to enable multisampling for anti-aliasing. In layman: make lines on openGL smooth and not "MS-Painted"
    QSurfaceFormat format;
    format.setDepthBufferSize(24);
    format.setSamples(16);
    QSurfaceFormat::setDefaultFormat(format);

    // Use GPU for the map if possible
